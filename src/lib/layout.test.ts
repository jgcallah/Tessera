...
describe("createLayoutItem", () => {
  it("creates an item with given position and size", () => {
    const item = createLayoutItem(0, 0, 2, 1, 3);
  ...
  it("generates unique IDs", () => {
    const a = createLayoutItem(0, 0, 1, 1, 3);
    const b = createLayoutItem(1, 0, 1, 1, 3);
...
describe("itemsOverlap", () => {
  it("returns true for overlapping items", () => {
    const a = createLayoutItem(0, 0, 2, 2, 3);
    const b = createLayoutItem(1, 1, 2, 2, 3);
    expect(itemsOverlap(a, b)).toBe(true);
  });

  it("returns false for non-overlapping items", () => {
    const a = createLayoutItem(0, 0, 2, 2, 3);
    const b = createLayoutItem(2, 0, 2, 2, 3);
  ...
  it("returns true for identical position items", () => {
    const a = createLayoutItem(0, 0, 1, 1, 3);
    const b = createLayoutItem(0, 0, 1, 1, 3);
    expect(itemsOverlap(a, b)).toBe(true);
  });
});
...
describe("canPlaceItem", () => {
  ...
  it("returns false if item extends beyond grid width", () => {
    const item = createLayoutItem(8, 0, 2, 1, 3); // starts at 8, width 2 → needs 10, grid is 9
    expect(canPlaceItem(item, state)).toBe(false);
  });

  it("returns false if item extends beyond grid height", () => {
    const item = createLayoutItem(0, 6, 1, 2, 3); // starts at 6, height 2 → needs 8, grid is 7
    expect(canPlaceItem(item, state)).toBe(false);
  });

  it("returns false if item overlaps existing item", () => {
    const existing = createLayoutItem(0, 0, 2, 2, 3);
    const stateWithItem: LayoutState = { ...state, items: [existing] };
    const newItem = createLayoutItem(1, 1, 1, 1, 3);
...
describe("addItem", () => {
  ...
  it("adds an item to empty layout", () => {
    const item = createLayoutItem(0, 0, 1, 1, 3);
  ...
  it("returns unchanged state if placement is invalid", () => {
    const item = createLayoutItem(9, 0, 1, 1, 3); // out of bounds
    const result = addItem(item, state);
  ...
  it("rejects overlapping item", () => {
    const a = createLayoutItem(0, 0, 2, 2, 3);
    const state1 = addItem(a, state);
    const b = createLayoutItem(1, 1, 1, 1, 3);
    const state2 = addItem(b, state1);
    expect(state2.items).toHaveLength(1); // b rejected
  });
});
...
describe("getPartsList", () => {
  ...
  it("groups identical items by size", () => {
    const state: LayoutState = {
      items: [
        createLayoutItem(0, 0, 2, 1, 3),
        createLayoutItem(2, 0, 2, 1, 3),
        createLayoutItem(4, 0, 2, 1, 3),
      ],
      gridUnitsX: 9,
      gridUnitsY: 7,
    };
    ...
    expect(parts[0]!.heightUnits).toBe(3);
  });

  it("separates items with different sizes", () => {
    const state: LayoutState = {
      items: [
        createLayoutItem(0, 0, 2, 1, 3),
        createLayoutItem(2, 0, 1, 1, 3),
        createLayoutItem(3, 0, 2, 1, 5),
      ],
      gridUnitsX: 9,
      gridUnitsY: 7,
    };
    const parts = getPartsList(state);
    expect(parts).toHaveLength(3); // all different
  });

  it("groups by all three dimensions (W × L × H)", () => {
    const state: LayoutState = {
      items: [
        createLayoutItem(0, 0, 2, 1, 3),
        createLayoutItem(0, 1, 1, 2, 3), // same area but different shape
      ],
      gridUnitsX: 9,
      gridUnitsY: 7,
    };
    const parts = getPartsList(state);
    expect(parts).toHaveLength(2); // different shapes
  });
});
